...
def selection_by_tournament(graph, population, k):  # seleção por torneio
    ...
    minor = float('inf')
    for i in range(k):
        if fitness(graph, selections[i]) < minor and i != i1 and selections[i][0] != selections[i1][0]:
            minor = fitness(graph, selections[i])
            i2 = i
...
def steady_stated(graph, population, filhos):
    ...
    menor = filhos[0]
    for i in range(localLen(filhos) - 1):
...
def genetic(graph, pop, mut, max_i, max_t, s, cross_operator):
    ...
    :param max_t: tempo máximo de execução em minutos
